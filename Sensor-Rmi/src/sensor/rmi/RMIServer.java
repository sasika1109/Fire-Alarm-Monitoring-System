...
public class RMIServer extends UnicastRemoteObject implements RMIService {
	...
	public String getSensorDetails() throws IOException, InterruptedException, RemoteException, JSONException {
		...
		if (responseCode == HttpURLConnection.HTTP_OK) {
			...
			JSONObject jsomnObj = new JSONObject(response.toString());// convert response string to JSON Object
			JSONArray sensors = jsomnObj.getJSONArray("sensors");// convert senros array to JSON array
                        
                        // convert each and every sensor to JSON Object
	...
	public String getSingleSensorDetails(String ID) throws IOException, InterruptedException, RemoteException {

		// assign passed ID
		final String SENSOR_ID = ID;

		// generate URL
		URL url = new URL("http://localhost:3000/api/sensors/" + SENSOR_ID);
	...
	public String addSensor(String FloorNo, String RoomNo, String Status)
			throws IOException, InterruptedException, RemoteException {
                ...
                // generate URL
		URL url = new URL(this.API_URL + "sensors/");
